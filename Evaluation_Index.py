...
def evaluate1(pre_path, label_path):
    ...
    for i, p in enumerate(pre_imgs):
        ...
        # print(type(imgPredict))
        # print(imgPredict.dtype)
        # print(imgLabel.shape)
        # print(type(imgLabel))
        # print(imgLabel.dtype)
...
if __name__ == '__main__':
    pre_path = 'data/test/result/'    # 生成结果
    label_path = 'data/test/label/'    # 真实标签

    # 计算测试集每张图片的各种评价指标，最后求平均
    acc_list, macc_list, mIoU_list, fwIoU_list = evaluate1(pre_path, label_path)
    print('result: acc(PA)={:.2f}%, macc(MPA)={:.2f}%, mIoU={:.2f}%, fwIoU={:.2f}%'
...
'''
    ...
    mIoU = metric.meanIntersectionOverUnion()
    fwIoU = metric.Frequency_Weighted_Intersection_over_Union()
    print('final2: acc(PA)={:.2f}%, macc(MPA)={:.2f}%, mIoU={:.2f}%, fwIoU={:.2f}%'
